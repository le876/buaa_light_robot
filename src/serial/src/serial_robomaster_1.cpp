...
public:
    serial_robomaster_1() : Node("serial_robomaster_1")
    {
        ...
        subscription_ = this->create_subscription<buaa_rescue_robot_msgs::msg::ControlMessageSlave>(
...
private: // 私有成员函数和变量
    ...
    // 处理接收到的Modbus帧
    std::tuple<std::array<int8_t, 12>, std::array<int32_t, 12>, int16_t> process_modbus_frame_for_snake_encorders(const std::vector<uint8_t> &frame)
    {
        ...
        {
            std::vector<uint8_t> data(frame.begin() + header.size(), frame.begin() + header.size() + 71);
        ...
        return std::make_tuple(snake_motor_encorder_speed_value, snake_motor_encorder_position_value, crc_verificated);
    }
                                      ...
                                      [this](const asio::error_code &error, std::size_t bytes_transferred)
                                      {
                                          ...
                                          if (!error)
                                          {
                                              ...
                                              if (it_snake_encorders != data_buffer_.end() && std::distance(it_snake_encorders, data_buffer_.end()) >= 79)
                                              {
                                                  ...
                                                  auto crc_verificated = std::get<2>(result);
    ...
    rclcpp::Subscription<buaa_rescue_robot_msgs::msg::ControlMessageSlave>::SharedPtr subscription_; // 订���器
