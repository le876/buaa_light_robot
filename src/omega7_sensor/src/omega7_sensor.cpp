...
#include "dhdc.h"
#include "drdc.h"
#include <Eigen/Dense> // 改为使用Dense而不是Eigen
...
public:
    ...
    Omega7Sensor(std::string name) : Node(name)
    {
        // 设置X11显示相关的环境变量
        setenv("DISPLAY", ":0", 1);
        setenv("QT_QPA_PLATFORM", "offscreen", 1); // 使用离屏渲染
        ...
        if (deviceCount > 0)
        {
            ...
            const char *leftName = dhdGetSystemName(left);
            const char *rightName = dhdGetSystemName(right);

            if (leftName)
            {
                printf("%s haptic device detected ID = %d\n\n", leftName, left);
                if (dhdIsLeftHanded(left))
                {
                    printf(" the device is configured for left-handed use\n");
                }
            }
        ...
        printf("BASE_LEFT  |  ");
        if (dhdHasGripper(left))
            printf("GRIP_LEFT  |  ");
        printf("BASE_RIGHT  |  ");
        if (dhdHasGripper(right))
            printf("GRIP_RIGHT  |  \n");
        printf("---------------------------------------------------------\n");
...
private:
    void timecaller()
    {
        ...
        drdGetPositionAndOrientation(&(p0[0]), &(p0[1]), &(p0[2]),
                                     ...
                                     &(p0[6]), r0, left);
        for (int i = 0; i < 3; i++)
            rotation0.row(i) = Vector3d::Map(&r0[i][0], 3);
        ...
        if ((normf0 = force0.norm()) > MAXF)
            force0 *= MAXF / normf0;

        // scale torque to a pre-defined ceiling
        if ((normt0 = torque0.norm()) > MAXT)
            torque0 *= MAXT / normt0;
        ...
        drdGetPositionAndOrientation(&(p1[0]), &(p1[1]), &(p1[2]),
                                     ...
                                     &(p1[6]), r1, right);
        for (int i = 0; i < 3; i++)
            rotation1.row(i) = Vector3d::Map(&r1[i][0], 3);
        ...
        // scale force to a pre-defined ceiling
        if ((normf1 = force1.norm()) > MAXF)
            force1 *= MAXF / normf1;

        // scale torque to a pre-defined ceiling
        if ((normt1 = torque1.norm()) > MAXT)
            torque1 *= MAXT / normt1;
