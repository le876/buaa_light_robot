...
private:
  void slave_control_topic_callback(const buaa_rescue_robot_msgs::msg::ControlMessageSlave::SharedPtr msg) {
    ...
    if(msg->robomaster_mode == 2 || msg->robomaster_mode == 12){ // Mode 2 and mode 12
      ...
      for (size_t i = 0; i < 12; i++)
      {
        // 当绳子张力处于设定区间的时候，让running_flag_1加一
      ...
      if (received_sensors_robomaster_1 && received_sensors_pull_push_1) {
        for (size_t i = 0; i < 12; i++)
        {
          ...
          if(last_sensors_pull_push_data_1.pull_push_sensors[i] < encorder_zero_down_limit_1[i]){
            ...
            if((msg->snake_position_control_array[i] - last_sensors_robomaster_data_1.snake_motor_encorder_position[i]) < delta){
              msg->snake_position_control_array[i] = last_sensors_robomaster_data_1.snake_motor_encorder_position[i] + delta;
            }
          }
          // backward
        ...
        if (running_flag_1 == 12 && msg->robomaster_mode == 2) {         
          ...
          for (size_t i = 0; i < 12; i++) {                
            if (i == 0 || i == 1 || i == 10 || i == 11) {            
                if (abs(last_sensors_pull_push_data_1.pull_push_sensors[i] - mean_tension_segment_1_1) > error_tension) {
                    msg->snake_speed_control_array[i] = average_speed + calculateAdjustedSpeed(last_sensors_pull_push_data_1.pull_push_sensors[i], mean_tension_segment_1_1, error_tension, average_speed, last_sensors_robomaster_data_1.snake_motor_encorder_speed[i]);
                }
            } else if(i == 2 || i == 3 || i == 8 || i == 9) {              
                if (abs(last_sensors_pull_push_data_1.pull_push_sensors[i] - mean_tension_segment_1_2) > error_tension) {
                    msg->snake_speed_control_array[i] = average_speed + calculateAdjustedSpeed(last_sensors_pull_push_data_1.pull_push_sensors[i], mean_tension_segment_1_2, error_tension, average_speed, last_sensors_robomaster_data_1.snake_motor_encorder_speed[i]);
                }
            } else if(i == 4 || i == 5 || i == 6 || i == 7) {
                if (abs(last_sensors_pull_push_data_1.pull_push_sensors[i] - mean_tension_segment_1_3) > error_tension) {
                    msg->snake_speed_control_array[i] = average_speed + calculateAdjustedSpeed(last_sensors_pull_push_data_1.pull_push_sensors[i], mean_tension_segment_1_3, error_tension, average_speed, last_sensors_robomaster_data_1.snake_motor_encorder_speed[i]);
                }
            }
          }
          // 然后发布新的控制消息
